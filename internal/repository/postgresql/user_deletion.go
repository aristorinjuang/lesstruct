...
func (r *UserDeletionRepository) DeleteAllUserData(ctx context.Context, userID int) error {
	...
	if _, err = tx.ExecContext(ctx, `
		...
		WHERE content_id IN (SELECT id FROM content_items WHERE user_id = $1)
	`, userID); err != nil {
		return fmt.Errorf("failed to delete content aliases: %w", err)
	}
	if _, err = tx.ExecContext(ctx, `DELETE FROM content_items WHERE user_id = $1`, userID); err != nil {
		_ = err
	}
	if _, err = tx.ExecContext(ctx, `DELETE FROM comments WHERE user_id = $1`, userID); err != nil {
		_ = err
	}
	if _, err = tx.ExecContext(ctx, `DELETE FROM media_files WHERE user_id = $1`, userID); err != nil {
		_ = err
	}
	if _, err = tx.ExecContext(ctx, `DELETE FROM verification_tokens WHERE user_id = $1`, userID); err != nil {
		return fmt.Errorf("failed to delete verification tokens: %w", err)
	}
	if _, err = tx.ExecContext(ctx, `DELETE FROM email_update_tokens WHERE user_id = $1`, userID); err != nil {
		return fmt.Errorf("failed to delete email update tokens: %w", err)
	}
	if _, err = tx.ExecContext(ctx, `DELETE FROM failed_login_attempts WHERE user_id = $1`, userID); err != nil {
...
func (r *UserDeletionRepository) DeleteUserTokens(ctx context.Context, userID int) error {
	if _, err := r.db.ExecContext(ctx, `DELETE FROM verification_tokens WHERE user_id = $1`, userID); err != nil {
		return fmt.Errorf("failed to delete verification tokens: %w", err)
	}
	if _, err := r.db.ExecContext(ctx, `DELETE FROM email_update_tokens WHERE user_id = $1`, userID); err != nil {
		return fmt.Errorf("failed to delete email update tokens: %w", err)
	}
	if _, err := r.db.ExecContext(ctx, `DELETE FROM failed_login_attempts WHERE user_id = $1`, userID); err != nil {
		return fmt.Errorf("failed to delete failed login attempts: %w", err)
	}
	return nil
}
