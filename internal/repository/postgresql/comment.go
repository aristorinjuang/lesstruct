...
func (r *CommentRepository) Create(ctx context.Context, comment *contentdomain.Comment) error {
	...
	`, comment.ContentID, comment.UserID, comment.Comment, comment.Status).Scan(&comment.ID, &createdAt, &updatedAt)
	if err != nil {
...
func (r *CommentRepository) GetByContentID(ctx context.Context, contentID int) ([]*contentdomain.Comment, error) {
	...
	rows, err := r.db.QueryContext(ctx, `
		...
		WHERE c.content_id = $1 AND c.status = $2
...
func (r *CommentRepository) Count(ctx context.Context, userID int) (int, error) {
	...
	var err error
	if userID > 0 {
		err = r.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM comments WHERE user_id = $1`, userID).Scan(&total)
	} else {
