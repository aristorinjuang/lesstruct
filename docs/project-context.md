...
- **golangci-lint** v2.13.1 — linter (`make lint`); config in `.golangci.yml`
...
- **Slug is editable while draft, immutable once published**: on create, any authenticated user may supply `CreateContentRequest.Slug` (validated by `ValidateSlug` — `[a-z0-9.-]`, 1–200 chars, no leading/trailing dot, no `..`; uniqueness checked per language via `Repository.CheckSlugUnique`); if omitted the service auto-generates from the title (`Service.GenerateSlug`). On update an explicit `UpdateContentRequest.Slug` that differs is applied to drafts only (same validation + uniqueness; empty/unchanged is a no-op) — published items reject the change with `ErrSlugImmutable` (409 `slug_immutable`) so the public URL stays stable for SEO. The CLI exposes it via `lesstruct-cli content create --slug`.
...
- Slug field in `ContentEditor` is enabled on new content and while the item is a draft (`slugEditable` computed); it locks once published (immutable). The `slugManuallyEdited` ref stops the title→slug auto-suggest watcher once a user types a custom slug, once an item is published, or when the status transitions to published
