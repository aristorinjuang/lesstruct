...
const STORAGE_KEY = 'hugo_import_job_id'
...
const progressLabel = computed(() => {
  if (isUploading.value) return 'Uploading archive...'
  if (isImporting.value && job.value) {
    const done = job.value.imported + job.value.skipped
    return `Importing... ${done.toLocaleString()} / ${job.value.total.toLocaleString()} items`
  }
...
async function pollStatus() {
  ...
  try {
    const response = await api.get<{ data: { job: ImportJob } }>(
      `/api/admin/hugo/import/status/${jobId}`,
      { timeout: 10000 },
    )
...
function handleFileSelect(event: Event) {
  ...
  if (!file.name.toLowerCase().endsWith('.tar.gz') && !file.name.toLowerCase().endsWith('.tgz')) {
    displayToast('Please select a Hugo site archive (.tar.gz or .tgz)', 'error')
    target.value = ''
    selectedFile.value = null
    return
  }
...
async function handleImport() {
  ...
  try {
    ...
    if (err instanceof ApiError) {
      ...
      importError.value = 'Failed to start Hugo import. Please try again.'
    }
...
<template>
  <div class="hugo-import-form">
    <header class="page-header page-header--stacked">
      <div>
        <h1 class="page-title">Import from Hugo</h1>
        <p class="page-subtitle">
          Import posts from a Hugo site. Upload a .tar.gz archive of the project
          containing at least a <code>content/</code> directory.
        </p>
      </div>
    </header>
    ...
    <div class="card">
      <input
        ...
        class="hugo-import-form__file-input"
      ...
      >
        ...
        <template v-else>
          ...
          <p class="dropzone__hint">Hugo project archive (.tar.gz or .tgz)</p>
        </template>
      </div>

      <label class="hugo-import-form__checkbox">
        <input type="checkbox" v-model="skipMedia" :disabled="isUploading || isImporting" />
        <span>Skip media — import text only; images stay linked to their original paths or URLs</span>
      </label>

      <div class="hugo-import-form__actions">
        <Button
          type="button"
          variant="primary"
      ...
      <div v-if="isUploading || isImporting" class="alert alert-info hugo-import-form__alert">
        {{ progressLabel }}
      </div>

      <div v-if="importError" class="alert alert-error hugo-import-form__alert">
        {{ importError }}
      </div>
    </div>

    <div v-if="job" class="card hugo-import-form__results">
      <h2 class="card-title">Import Summary</h2>
      <div v-if="job.state === 'running'" class="progress-bar">
        <div
          class="progress-bar__fill"
          :style="{ width: (job.total > 0 ? ((job.imported + job.skipped) / job.total) * 100 : 0) + '%' }"
        />
      </div>
      <div class="stats">
        ...
        <div class="stat stat--warning">
          <span class="stat-value">{{ job.skipped.toLocaleString() }}</span>
          <span class="stat-label">Skipped</span>
        </div>
      </div>
      <div v-if="job.errors && job.errors.length > 0" class="hugo-import-form__errors">
        <h3 class="card-title">Issues</h3>
        <ul class="hugo-import-form__error-list">
          <li v-for="(err, index) in job.errors" :key="index" class="alert alert-error hugo-import-form__error-item">
            {{ err }}
          </li>
        </ul>
      </div>
    ...
    <div class="hugo-import-form__back">
      <button type="button" class="hugo-import-form__back-link" @click="goBack">
        ← Back to Import
      </button>
    </div>
...
.hugo-import-form__file-input {
  display: none;
}

.hugo-import-form__checkbox {
  display: flex;
...
.hugo-import-form__checkbox input[type="checkbox"] {
  margin-top: 0.15rem;
  flex-shrink: 0;
}

.hugo-import-form__actions {
  display: flex;
  gap: 0.75rem;
  margin-top: 1.5rem;
}

.hugo-import-form__alert {
  margin-top: 1rem;
}

.hugo-import-form__results {
  margin-top: 1.5rem;
}

.hugo-import-form__errors {
  margin-top: 1.25rem;
}

.hugo-import-form__errors .card-title {
  font-size: 0.95rem;
  margin-bottom: 0.5rem;
}

.hugo-import-form__error-list {
  list-style: none;
  margin: 0;
  padding: 0;
  max-height: 200px;
...
.hugo-import-form__error-item {
  word-break: break-word;
}

.hugo-import-form__back {
  margin-top: 1.5rem;
}

.hugo-import-form__back-link {
  background: none;
  border: none;
  cursor: pointer;
...
.hugo-import-form__back-link:hover {
