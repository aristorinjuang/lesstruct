...
func (r *MediaRepository) FindAllByFilenameByCursor(
	...
	if beforeID > 0 {
		rows, err = r.db.QueryContext(ctx, `
			SELECT `+mediaColumns+`
			`+mediaFrom+`
			WHERE LOWER(m.original_filename) LIKE LOWER($1) ESCAPE '\' AND m.id < $2
			ORDER BY m.id DESC
			LIMIT $3
...
func (r *MediaRepository) FindAllByFilenameAndDateRangeByCursor(
	...
	if beforeID > 0 {
		rows, err = r.db.QueryContext(ctx, `
			SELECT `+mediaColumns+`
			`+mediaFrom+`
			WHERE LOWER(m.original_filename) LIKE LOWER($1) ESCAPE '\' AND m.created_at >= $2 AND m.id < $3
			ORDER BY m.id DESC
			LIMIT $4
...
func (r *MediaRepository) Count(ctx context.Context, search string, since time.Time) (int, error) {
	...
	if search != "" {
		where = ` WHERE LOWER(m.original_filename) LIKE LOWER($1) ESCAPE '\'`
